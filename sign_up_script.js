...
function validatePhone() {
    var phone = document.getElementById('floatingInputContact').value;
    var contactBox = document.getElementById('floatingInputContact');

    if (phone.length == 0) {
        phoneError.innerHTML = '*contact number is required';
        contactBox.style.borderColor = "#cc0000";
        contactBox.style.boxShadow = "0px 0px 10px 2px #ffcccc";
        return false;
    }
...
function validateFname() {
    var fname = document.getElementById('floatingInputFname').value;
    var fnameBox = document.getElementById('floatingInputFname');

    if (fname.length == 0) {
        fnameError.innerHTML = '*required';
        fnameBox.style.borderColor = "#cc0000";
        fnameBox.style.boxShadow = "0px 0px 10px 2px #ffcccc";
        return false;
    }
    if (!fname.match(/^([a-zA-Z])+$/)) {
        ...
        fnameBox.removeAttribute("style");
        return true;
    }
}
...
function validateLname() {
    var lname = document.getElementById('floatingInputLname').value;
    var lnameBox = document.getElementById('floatingInputLname');

    if (lname.length == 0) {
        lnameError.innerHTML = '*required';
        lnameBox.style.borderColor = "#cc0000";
        lnameBox.style.boxShadow = "0px 0px 10px 2px #ffcccc";
        return false;
    }
    if (!lname.match(/^([a-zA-Z])+$/)) {
        lnameError.innerHTML = '*invalid format';
        lnameBox.style.borderColor = "#cc0000";
        lnameBox.style.boxShadow = "0px 0px 10px 2px #ffcccc";
        return false;
    } else {
        lnameError.innerHTML = '<i class="fa-solid fa-circle-check"></i>';
        lnameBox.removeAttribute("style");
    }
}